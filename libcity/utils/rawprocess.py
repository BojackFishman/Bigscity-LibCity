...
def generate_graph(grid_list, traj_list, user_list, user_traj_dict, user_traj_train):
    ...
    for key in user_traj_dict:
        for one_traj in user_traj_dict[key]:
            one_traj_list = one_traj[0]     # 这里应该取网格序列，是轨迹数据的第0列
    ...
    for key in user_traj_dict:
        ...
        for one_traj in user_traj_dict[key]:
            for idx in one_traj[0]:     # 网格序列，encoded后的数据中是第0列数据
                global_feature[one_traj[3][0], idx] += 1
            sum_feature += global_feature[one_traj[3][0]]      
            # 前两行，应该是取轨迹序号，在encoded后的数据中是第3列数据，列表中只有唯一值
        global_feature[len(traj_list)+int(key)] = sum_feature
    ...
    for key in user_traj_train:
        node1 = len(traj_list) + int(key)
        for one_traj in user_traj_train[key]:
            node2 = one_traj[3][0]      # 取轨迹序号
